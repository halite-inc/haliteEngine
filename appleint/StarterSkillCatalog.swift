...
enum StarterSkillCatalog {
    ...
    static let exposedSkills: [CustomSkill] = [
        ...
        CustomSkill(id: internetSearchID, name: "Internet Search", summary: "Source-first web retrieval for substantive answers, with current multi-source evidence and citations. Edit this to customize how the AI uses search.", instructions: "internet_use(query); before calling it, interpret the user's full request and identify only the current, uncertain, or externally verifiable facts that actually need checking. Formulate a laser-focused, concise search query containing the exact keywords, product names, topics, or entities needed for search engines (e.g. 'best AAA games 2026' rather than full conversational sentences); do not expose or narrate private reasoning. For latest/current/recommendation/price/compatibility requests, use the current system year unless the user chose another year. Inspect coverage after each result: if a material requested facet is weak or unsupported, search again with a meaningfully different targeted query for that gap. The backend searches multiple providers, fetches diverse websites, and synthesizes evidence."),
