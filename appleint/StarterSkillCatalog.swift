...
enum StarterSkillCatalog {
    ...
    static let evidenceFirstSearchRule = " EVIDENCE-FIRST PRIORITY: SOURCE-FIRST WEB DEFAULT. For every substantive knowledge request, retrieve current web evidence before answering even when the user did not explicitly ask to search. Convert conversational wording into focused evidence targets; never use the user's whole sentence as a query merely to preserve constraints. Decompose multi-entity or multi-facet requests when separate evidence is required, choose source types suited to the domain, run independent focused queries concurrently, and verify coverage for each query. Use authoritative standard-unit facts for calculations and perform the user's exact arithmetic after retrieval. Read fetched page evidence, reconcile conflicts, and give a direct synthesis rather than dumping snippets. Prefer primary/official sources for technical, medical, legal, financial, and factual claims; add independent sources for comparisons, recommendations, products, news, and disputed claims. Place links beside supported claims and label inference or missing coverage. Skip automatic search only for very basic conversation, an explicit opt-out, local clock answers, user-supplied/local workspace work, or a request explicitly owned by another tool such as files, tasks, forms, or visualizations. Preserve exact names, models, versions, dates, places, quantities, and scope."
    ...
    static let exposedSkills: [CustomSkill] = [
        ...
        CustomSkill(id: internetSearchID, name: "Internet Search", summary: "Source-first web retrieval for substantive answers, with current multi-source evidence and citations. Edit this to customize how the AI uses search.", instructions: "internet_use(query); before calling it, interpret the user's full request and identify only the current, uncertain, or externally verifiable facts that actually need checking. Form a focused query for those facts while preserving every exact name/model/chip/date/quantity from the user; do not expose or narrate private reasoning. For latest/current/recommendation/price/compatibility requests, use the current system year unless the user chose another year. Never introduce an old year from memory or replace an unfamiliar product with an older generation. Inspect coverage after each result: if a material requested facet is weak or unsupported, search again with a meaningfully different targeted query for that gap. Never repeat a query; use at most four distinct search rounds, then answer with explicit limitations. The backend searches multiple providers, fetches diverse websites, and may automatically refine weak discovery."),
        ...
        CustomSkill(id: filesystemID, name: "Terminal & Filesystem", summary: "Toolbox capability for validated terminal commands and local file operations.", instructions: "file_system(action,path,content,command,files); exact actions: execute_command,list,read_file,create_file,create_files,create_folder; use create_files for up to 12 related project files; Home: {{HOME}}; Downloads: {{HOME}}/Downloads; Documents: {{HOME}}/Documents; Desktop: {{HOME}}/Desktop"),
        CustomSkill(id: toolRouterID, name: "Tool Execution Contract", summary: "Core rules governing how the AI emits, chains, and completes tool calls.", instructions: "To act, emit exactly one valid JSON object with a `type` matching a listed tool and only its needed fields. Tool calls are intermediate: inspect the structured result and either emit the single next action explicitly required by the user or a concise final answer. Stop immediately once the requested result is confirmed; never add unrequested actions or narrate private reasoning. Request input when needed and wait. Never invent tool names, field types, or action names."),
